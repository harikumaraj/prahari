...
export default class TempGraph extends Component{
    ...
    constructor(props) {
        ...
        for (let index in this.props.tempMax) {
            valMax.push(this.props.tempMax[index].y);
            valMin.push(this.props.tempMin[index].y);
    ...
    componentWillMount(){
        if(parseInt(this.state.days)===1){
            ...
            let valMin = ["Minimum"];
            let tableData = ["Difference"];
            let interval=this.state.tempMin.length/24;
            for(let i=0;i<24;i++){
                temporaryDataDayOne.push({x: i + 1, y: this.state.tempMax[Math.floor(i * interval)].y});
                temporaryTempMaxDayOne.push({x: i + 1, y: this.state.tempMax[Math.floor(i * interval)].y});
                temporaryTempMinDayOne.push({x: i + 1, y: this.state.tempMax[Math.floor(i * interval)].y});
                temporaryTimeListDayOne.push(this.state.dateList[Math.floor(i * interval)]);
                valMax.push(this.state.tempMax[Math.floor(i * interval)].y);
                valMin.push(this.state.tempMin[Math.floor(i * interval)].y);
            ...
            this.setState({
                ...
                tempMax:temporaryTempMaxDayOne,
                tempMin:temporaryTempMinDayOne,
    ...
    tableForOneOrMultipleDays() {
        if (this.state.days === "1") {
            let data = this.state.tempMaxTable;
            data.shift();
            return (
                <View style={{flex: 1}}>
                    ...
                    <View style={{flex: 1, flexDirection: "row"}}>
                        ...
                        <Text style={{
                            ...
                            {(Math.max(...data)-Math.min(...data)).toFixed(1)}
                        </Text>
                    </View>
                </View>
            ...
            return (
                <View style={{flex:1}}>
                    ...
                    <View style={{flex:1,flexDirection:"row"}}>
                        {
                            this.state.tempMaxTable.map((temp,index)=>
                                <View key={index} style={{
                                    width:100,
                                    height:50,
                                    alignItems:"center",
                    ...
                    <View style={{flex:1,flexDirection:"row"}}>
                        {
                            this.state.tempMinTable.map((temp,index)=>
                                <View key={index} style={{
                                    width:100,
                                    height:50,
                                    alignItems:"center",
